...
def generate_essay_content(details):
    ...
    if choice == "1":
        body_p = f"Write a full body for an essay on {details['topic']} with stance of {details['target_audience']}"
        body = generate_response(body_p, temperature=temp, max_tokens=1024)
        print("\n=== Generated Full Body ===\n")
        print(body)
    else:
        step_p = f"Write step_by_step arguments for an essay on {details['topic']}. Provide evidence and reasoning."
        step = generate_response(step_p, temperature=temp, max_tokens=1024)
