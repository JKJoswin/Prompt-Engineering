...
def reinforcement_learning_activity():
    ...
    prompt = input("Enter a prompt for the AI (e.g., Decribe the lion):").strip()
    if not prompt:
    ...
    print("2. How does reinforcement learning helpAI to improve its performance over time?")

def role_based_prompt_activity():
    print("\n=== ROLE-BASED PROMPTS ACTIVITY ===\n")
    category = input("Enter a category (e.g., science, math, history):").strip()
    item = input(f"Enter a specific {category} topic (e.g., 'photosynthesis' for science):").strip()

    if not category or not item:
        print("Fill in both fields to run the activity.")
        return
    ...
    print("\nReflection:")
    print("1. How did AI's response differ between teacher's and expert's perspectives?")
    print("2. How can role-based prompts help tailor AI responses for different contexs?")
